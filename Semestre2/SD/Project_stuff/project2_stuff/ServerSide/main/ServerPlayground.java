...
public class ServerPlayground {
   ...
   *         args[2] - port nunber where the server for the general repository is listening to service requests
   ...
   {
      MPlayground mPlayground;                                                 // Playgroud (service to be rendered)
      IPlayground iPlayground;                                                 // interface to the playground
     ...
     reposStub = new SGeneralRepo(reposServerName, reposPortNumb);       // communication to the general repository is instantiated
      mPlayground = new MPlayground (reposStub);                                      // service is instantiated
     iPlayground = new IPlayground (mPlayground);                            // interface to the service is instantiated
    ...
    { try
      { sconi = scon.accept ();                                    // enter listening procedure
        cliProxy = new PlaygroundClientProxy (sconi, iPlayground);    // start a service provider agent to address
