...
public class ServerRefereeSite {
   ...
   *         args[2] - port nunber where the server for the general repository is listening to service requests
   ...
   {
      MRefereeSite mRefereeSite;                                                 // Referee Site (service to be rendered)
      IRefereeSite iRefereeSite;                                                 // interface to the Referee Site
     ...
     mRefereeSite = new MRefereeSite (reposStub);                                      // service is instantiated
     iRefereeSite = new IRefereeSite (mRefereeSite);                            // interface to the service is instantiated
    ...
    { try
      { sconi = scon.accept ();                                    // enter listening procedure
        cliProxy = new RefereeSiteClientProxy (sconi, iRefereeSite);    // start a service provider agent to address
